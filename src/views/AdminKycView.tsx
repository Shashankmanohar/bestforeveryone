    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-900">KYC Requests</h2>
                    <p className="text-sm text-gray-500">Manage user document verifications</p>
                </div>
            ...
            {pendingUsers.length === 0 ? (
                <div className="bg-white rounded-3xl border border-gray-200 p-20 text-center shadow-card">
                    <Icon icon="solar:check-read-linear" className="mx-auto text-gray-300 mb-4" width={48} />
                ...
                <div className="grid gap-4">
                    {pendingUsers.map((user) => (
                        <motion.div
                            ...
                            className="bg-white rounded-2xl border border-gray-200 p-5 shadow-card overflow-hidden"
                        >
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                <div className="flex items-center gap-4">
                                    <div className="h-12 w-12 bg-gray-100 rounded-full flex items-center justify-center font-bold text-gray-600 capitalize text-lg">
                                        {user.fullname[0]}
                                    </div>
                                    <div>
                                        <h3 className="font-bold text-gray-900">{user.fullname}</h3>
                                        <p className="text-xs text-gray-500 tracking-tight">@{user.username} • {user.phone}</p>
                                        <p className="text-[10px] text-gray-400 font-bold uppercase mt-1">Submitted: {new Date(user.kyc.submittedAt).toLocaleDateString()}</p>
                                    </div>
                                </div>
                                ...
                                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3">
                                    <a
                                        ...
                                        className="flex items-center gap-2 text-[10px] font-bold text-blue-600 bg-blue-50 px-3 py-2 rounded-lg border border-blue-100 hover:bg-blue-100 transition-colors"
                                    >
                                    ...
                                    <a
                                        ...
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="flex items-center gap-2 text-[10px] font-bold text-amber-600 bg-amber-50 px-3 py-2 rounded-lg border border-amber-100 hover:bg-amber-100 transition-colors"
                                    >
                                        <Icon icon="solar:bank-card-bold" /> PAN Card
                                    </a>
                                    <div className="col-span-2 bg-gray-50 rounded-lg p-2 border border-gray-100 text-[10px]">
                                        <p className="text-gray-400 uppercase font-bold mb-1">Bank Account</p>
